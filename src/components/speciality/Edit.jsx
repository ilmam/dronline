...
import {
  ...
  Select, notification, Image, Tooltip,
} from 'antd';
import {
  SendOutlined,
  CheckOutlined,
...
class Edit extends Component {
  constructor() {
    ...
    this.fetchData = (id) => {
      superagent.get(`/admin/speciality/${id}`)
        .end((err, res) => {
            ...
            if (this.formRef.current !== null) {
              this.formRef.current.setFieldsValue({
                ...
                parent_id: body.parent_id !== 0
                  ? { key: body.parent_id, label: body.parent_name_en } : undefined,
                active: body.active,
              });
              this.setState({
    ...
    this.onFinish = (values) => {
      ...
      superagent
        .put(`/admin/speciality/${resourceId}`)
        .send(data)
        .end((err) => {
          if (!err) {
