...
class ItemList extends Component {
  constructor() {
    ...
    this.isActive = (value) => {
      ...
      superagent
        .patch(`/admin/packageitem/${value.id}`)
        .send({ active })
        .end((err) => {
          if (!err) {
            this.fetchData();
    ...
    this.columns = [
      ...
      {
        title: 'Package name',
        dataIndex: 'package_name_en',
        key: 'package_name_en',
        sorter: true,
        ...this.getColumnSearchProps('package_name_en'),
        render: (value) => (
          <div>{value || '---'}</div>
        ),
      },
    ...
    this.fetchData = () => {
      ...
      superagent
        .get(`/admin/packageitem/grid?${qs.stringify(params)}`)
        .end((err, res) => {
          if (!err) {
            const { body } = res;
