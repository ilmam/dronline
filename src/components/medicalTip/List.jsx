...
class PackageList extends Component {
  constructor() {
    ...
    this.isActive = (value) => {
      ...
      superagent
        .patch(`/admin/medicaltip/${value.id}`)
        .send({ active })
        .end((err) => {
          if (!err) {
            this.fetchData();
    ...
    this.columns = [
      ...
      },
      {
        ...
        render: (value) => (
          <div>{moment(value).format(dateFormat)}</div>
        ),
      },
      {
        title: 'To Date',
      ...
      },
      {
        ...
        render: (object) => (
          <Button.Group size="middle">
            <Button
              onClick={() => this.editBtnClicked(object)}
              type="dashed"
              icon={<FormOutlined />}
              shape="round"
            />
    ...
    this.fetchData = () => {
      ...
      superagent
        .get(`/admin/medicaltip/grid?${qs.stringify(params)}`)
        .end((err, res) => {
          if (!err) {
            const { body } = res;
  ...
  render() {
    const {
      data, pagination, loading, editResourceId, rowData,
    } = this.state;
    return (
      <div>
        <Modal
