...
class List extends Component {
  constructor() {
    ...
    this.onChangeUserStatus = (value) => {
      const { userStore } = this.props;

      if (userStore.value.id === value.id && value) {
        const c = this;
        AntModal.confirm({
          title: 'Warning!',
          content: 'The user you\'re trying to change its state is the user that you\'re currently logged in with! and if you proceed you may experience logging out or unable to do most of the site functionality\'s!',
          okText: 'Proceed',
          okType: 'danger',
          centered: 'centered',
          onOk() {
    ...
    this.onDeleteUser = (Id) => {
      ...
      if (userStore.value.id === Id) {
        const c = this;
        AntModal.confirm({
          title: 'Warning!',
          content: 'The user you\'re trying to delete is the user that you\'re currently logged in with! and if you proceed you may experience logging out or unable to do most of the site functionality\'s!',
          okText: 'Proceed',
          okType: 'danger',
          centered: 'centered',
          onOk() {
        ...
        render: (record, value) => (
          <>
            <Button
              type="text"
              shape="round"
              onClick={() => this.onChangeUserStatus(value)}
              icon={record
      ...
      },
      {
        ...
        render: (object) => (
          <Button.Group size="middle">
            ...
            />
            <Popconfirm
              title={`Are you sure delete [${object.name}]`}
              onConfirm={() => { this.onDeleteUser(object.id); }}
              okText="Yes"
              cancelText="No"
    ...
    this.fetchData = () => {
      ...
      superagent
        .get(`/admin/user/grid?${qs.stringify(params)}`)
        .end((err, res) => {
          if (!err) {
            const { body } = res;
